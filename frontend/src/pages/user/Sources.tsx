...
import { deleteUploadedSource, getSources, getUploads, unmountSource } from '../../api/sources'
...
export default function Sources() {
    ...
    Promise.allSettled([getSources(), getUploads()])
      .then(([sourcesResult, uploadsResult]) => {
        ...
        if (uploadsResult.status === 'fulfilled') {
          ...
          setUploads([])
          setMessage(getApiErrorMessage(uploadsResult.reason, 'Unable to load pending sources.'))
        }
      })
  ...
  const handleUnmount = async (source: Source) => {
    ...
    setMessage('')
    try {
      const res = await unmountSource(source.name)
  ...
  const handleDeleteUpload = async (source: UploadedSource) => {
    ...
    setMessage('')
    try {
      const res = await deleteUploadedSource(source.filename)
      setMessage(res.message)
      load()
    } catch (err) {
  ...
  const confirmDescription = confirmAction?.type === 'delete-upload'
    ? `Confirm deletion of pending source "${confirmAction.source.displayName}"?`
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl">
      <PageHeader
        ...
        actions={(
          <LoadingButton
            ...
            loading={false}
            className="h-[38px] self-end px-4 border border-blue-200 bg-white text-blue-700 hover:bg-blue-50 text-sm font-medium rounded-lg transition-colors"
          >
      ...
      {loading ? (
        ...
        <EmptyState message="No source available." className="py-16" />
      ) : (
        <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {results.map((source) => (
                    <DatasetCard
                      key={source.id}
                      source={source}
                      onDescribe={() => navigate(`/app/describe?id=${encodeURIComponent(source.name)}&run=1`)}
                      onProfile={() => navigate(`/app/profile?id=${encodeURIComponent(source.name)}&run=1`)}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                  {uploadResults.map((source) => (
                    <UploadedSourceCard
                      key={source.id}
                      source={source}
                      onMount={() => navigate(`/app/mount?uploaded=${encodeURIComponent(source.filename)}`)}
